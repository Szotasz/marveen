...
A B2B admin UI (dashboard "Felhasználók" oldal, csak globális adminoknak látható) háromfüles kezelőfelületet ad: tenantok listázása/letiltása/törlése, felhasználók létrehozása/szerkesztése (display_name, email, role)/letiltása/törlése tenant-szűrővel, eszközkulcsok tenant-hozzárendelése. `DELETE /api/admin/users/:id` hard-delete önvédelemmel (nem törölheted saját fiókodat) és utolsó-admin guarddal. Az ágensek elérhetőségét tenant-szinten deny-by-default elven lehet engedélyezni (`tenant_agent_availability` tábla, `GET/PUT /api/admin/agent-availability`); ha egy tenanthoz nincs engedélyező sor, az ágens alapértelmezés szerint letiltottnak számít.
