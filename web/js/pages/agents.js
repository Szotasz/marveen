...
// openModal / closeModal -> imported from /js/core/ui.js
...
// showToast -> imported from /js/core/ui.js
...
function renderAgents() {
  ...
  if (window._marveen) {
    ...
    mCard.querySelector('.agent-terminal-btn')?.addEventListener('click', (e) => {
      e.stopPropagation(); window.openTerminalModal?.(mainAgentId())
    })
    mCard.querySelector('.agent-conversation-btn')?.addEventListener('click', (e) => {
      e.stopPropagation(); window.openConversationModal?.(mainAgentId(), t('agents.marveen_boss'))
    })
  ...
  for (const agent of agents) {
    ...
    card.querySelector('.agent-terminal-btn')?.addEventListener('click', (e) => {
      e.stopPropagation(); window.openTerminalModal?.(agent.name)
    })
    // Conversation (readable transcript) button
    card.querySelector('.agent-conversation-btn')?.addEventListener('click', (e) => {
      e.stopPropagation(); window.openConversationModal?.(agent.name, label)
    })
...
document.getElementById('voiceInstallBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    const VOICE_INSTALL_MAX_POLLS = 80
...
document.getElementById('addSkillBtn').addEventListener('click', () => {
  setSkillModalScope(null)  // per-agent flow keyed off currentAgent
