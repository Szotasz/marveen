...
export const DEFAULT_CONTEXT_GUARD: ContextGuardConfig = {
  ...
  // minutes), so it is the fleet's evidenced "quieter than this is not a
  // working turn" boundary rather than a second invented figure.
  idleMinutes: 20,
}
 ...
 * loses the thread it was holding) dwarfs the cost of a wrong "no" (the fleet
 * pays for one more heavy session until the next sweep).
 *
