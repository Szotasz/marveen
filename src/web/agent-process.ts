...
// Non-blocking wait. The delivery path (sendPromptToSession) and its helpers
// used execFileSync('/bin/sleep', ...) which froze the libuv event loop -- the
// per-chunk 0.03s pacing over a long message could block the loop for seconds,
// and the message router drives this every 5s. Awaiting a setTimeout yields the
// loop instead of blocking it, keeping the dashboard HTTP server responsive.
...
export async function waitForPaneIdle(
  ...
  for (;;) {
    ...
    await delay(Number(PANE_IDLE_POLL_S) * 1000)
  }
}
...
async function discardPlaceholderBuffer(session: string, host: string | null = null): Promise<boolean> {
  for (let i = 0; i < PLACEHOLDER_DISCARD_MAX; i++) {
    ...
    await delay(Number(PLACEHOLDER_DISCARD_SETTLE_S) * 1000)
  }
...
export async function sendPromptToSession(
  ...
  for (let attempt = 0; ; attempt++) {
    await delay(Number(SUBMIT_RETRY_POLL_MS) * 1000)
    const pane = capturePane(session, host)
...
export async function isSessionReadyForPrompt(session: string, host: string | null = null): Promise<boolean> {
  ...
  await delay(Number(PANE_READY_CONFIRM_DELAY_S) * 1000)
...
export async function clearStaleParkedInput(session: string, host: string | null = null): Promise<boolean> {
  ...
  await delay(Number(PARKED_STABLE_CONFIRM_S) * 1000)
  ...
  for (let i = 0; i < PARKED_CLEAR_MAX; i++) {
    runTmux(host, ['send-keys', '-t', session, 'C-u'], { timeout: 5000 })
    await delay(Number(PARKED_CLEAR_SETTLE_S) * 1000)
