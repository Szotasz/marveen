...
export async function tryHandleConnectors(ctx: RouteContext): Promise<boolean> {
  ...
  if (path === '/api/connectors/external-paths' && method === 'POST') {
    ...
    if (result.error) { json(res, { error: result.error, ...(result.hint ? { hint: result.hint } : {}), ...(result.field ? { field: result.field } : {}) }, 400); return true }
  ...
  if (githubRepoMatch && method === 'PATCH') {
    ...
    if (result.error) { json(res, { error: result.error, ...(result.hint ? { hint: result.hint } : {}) }, 400); return true }
    json(res, { ok: true })
    return true
  }
