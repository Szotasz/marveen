...
export async function synthesizeVoiceReply(text: string): Promise<{ audioBase64: string; mimeType: string } | null> {
  // Split the TTS cost so the latency breakdown shows token-mint vs pure generation.
  // A standalone probe (2026-07-01) measured mint ~237ms ONCE (then cached) and generation
  // ~4.7s for a 45-char sentence, scaling with reply length -- so a long production ttsMs
  // is generation (i.e. reply length), NOT a per-call token fetch. textChars is logged so
  // length<->latency stays visible in prod.
  const tMintStart = Date.now()
  const token = await mintAccessToken()
  const tokenMintMs = Date.now() - tMintStart
  ...
  const ttsFetchMs = Date.now() - tFetchStart
  logger.info({ tokenMintMs, ttsFetchMs, textChars: text.length }, 'voice-live: TTS timing')
