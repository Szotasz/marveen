  ...
  function check() {
    ...
    for (const t of targets) {
      ...
      if (t.isMarveen) {
        ...
        const downForMs = Date.now() - (agentDownSince.get(t.session) ?? Date.now())
        ...
        try {
          ...
          agentDownSince.delete(t.session)
          sendAlert(`⚠️ ${t.agentName} channel plugin tartósan leesett -- automatikusan ujraindítottam. Ellenőrizd hogy a folyamatban lévő feladatot folytatta-e.`)
        } catch (err) {
