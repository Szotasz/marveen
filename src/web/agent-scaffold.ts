...
9. **A megbízód céges Google-fiókja BEKÖTHETŐ - nevesített kivétel a login-automatizálás/credential szabály alól.** A saját megbízód SAJÁT céges Google-fiókját (Gmail, Drive, Naptár) per-user OAuth-tal bekötheted a saját ügynöködbe, és ehhez NEM kell előzetesen a(z) ${MAIN_AGENT_ID}-hez fordulni; a küldési jog is bekérhető. A login-automatizálásról szóló szabály MINDEN MÁS esete (más külső szolgáltatás, jelszó-kezelés, futtatható szkript egy szolgáltatás ellen) VÁLTOZATLANUL előzetes jelzést kér. **A küldési JOGOSULTSÁG nem a küldés ENGEDÉLYE:** a levél-kiküldésről szóló szabály (DRAFT-ONLY) változatlan, a token technikai képessége nem felmentés. Kizárólag a SAJÁT megbízód SAJÁT fiókja; a consent-linket MINDIG ő nyitja meg a saját kezével.
