...
function buildAutonomyBody(name: string): string {
  return [
    ...
    'status=approved -> mehet a művelet, de ELŐTTE fogyaszd el a jóváhagyást (lásd alább). status=rejected vagy status=timeout -> ne csináld, naplózd az okot.',
    '',
    'Fogyasztás (POST, KÖTELEZŐ a cselekvés ELŐTT):',
    `curl -s -X POST ${dashboardOrigin}/api/approvals/<id>/consume -H "Content-Type: application/json" -H "Authorization: Bearer $(cat ${tokenPath})" -d '{"actor":"${name}"}'`,
    'HTTP 200 -> végezd el a műveletet. HTTP 409 -> NE cselekedj, a `verdict` megmondja miért: `pending` (még nincs döntés), `consumed` (ezt az igent MÁR felhasználták), `expired` (a döntés kívül van a 24 órás ablakon), `rejected`, `timeout`, `hash_mismatch` (a jóváhagyás MÁS tartalomra szól), `race` (egy párhuzamos futás vitte el).',
    'MIÉRT: egy `approved` sor korábban örök engedély volt. Mérve 2026-09-23-án egy 09-20-i igen három nap után is felhasználhatónak látszott, és semmi nem kötötte ahhoz, amire adták. Az a jóváhagyás, amit nem lehet elhasználni, nem jóváhagyás, hanem nyitott kapu.',
    'Ha a művelet KONKRÉT tételekre szól (például memória-ID-k, fájlok), kötözd tartalomhoz: a kérésnél ÉS a fogyasztásnál add meg ugyanazt a `content_hash`-t, a rendezett tétel-lista sha256-ját. Így egy régi igen nem tud egy MÁS halmazra érvényes lenni, a szerver `hash_mismatch`-el elutasítja.',
    'A hash képzése: `printf \'%s\' \'124,129,168\' | shasum -a 256 | cut -d\' \' -f1`. A tételeket rendezd, és a kérés leírásában is sorold fel őket, hogy utólag eldönthető legyen, mire szólt az igen.',
    '',
