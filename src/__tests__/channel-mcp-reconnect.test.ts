...
describe('attemptChannelMcpReconnect', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    // The reconnect sequence now paces with `await delay(ms)` (non-blocking)
    // instead of the old blocking execFileSync('/bin/sleep', ...). Fake timers
    // keep the tests instant and deterministic: each test drives the promise to
    // completion with vi.runAllTimersAsync() (see runReconnect).
    vi.useFakeTimers()
  ...
  // Kick off the (now-async) reconnect and flush all pending delay() timers so
  // it runs to completion synchronously under fake timers.
  async function runReconnect(agent: string): Promise<{ ok: boolean; message: string }> {
    const p = attemptChannelMcpReconnect(agent)
  ...
  it('defers (sends NO keys) when the pane is actively generating', async () => {
    ...
    const result = await runReconnect('marveen')
