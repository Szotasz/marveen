...
describe('the real launch builder escapes the model AT the sink (not only at the validator)', () => {
  const OPTS = { claudePath: 'claude', pluginId: 'telegram', continueSession: false, config: mainConfigDecisionForTest(), stateDir: { envVar: 'TELEGRAM_STATE_DIR', dir: '/install/.claude/channels/telegram' } }
