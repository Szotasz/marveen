...
import { bridgeEnroll, RemoteEnrollError, type BridgeEnrollDeps } from '../web/bridge-enroll.js'
import { removeBridgeSshAccess } from '../remote-enroll-fs.js'
...
describe('bridgeEnroll', () => {
  it('writes the restricted line, mints a paired device key, and embeds it in the bundle', async () => {
    ...
    const outcome = await bridgeEnroll({ keyLine: line, name: 'device-a' }, testDeps())
    ...
    expect(keys[0]).toMatchObject({ name: 'device-a', installId })
  ...
  it('re-pairing the same installId replaces BOTH the ssh line and the device key', async () => {
    const { line, installId } = makeKeyLine()
    const first = await bridgeEnroll({ keyLine: line, name: 'device-b' }, testDeps())
    const second = await bridgeEnroll({ keyLine: line, name: 'device-b-repaired' }, testDeps())
    ...
    expect(listDeviceKeys()[0]!.name).toBe('device-b-repaired')
  ...
  it('preserves foreign authorized_keys lines byte-for-byte', async () => {
    ...
    const foreign = 'ssh-ed25519 AAAAforeignkey user@host\n'
...
describe('POST /api/security/bridge-enroll (HTTP)', () => {
  it('denies device and federation principals, and a missing principal', async () => {
    for (const auth of [
      { kind: 'device', device: 'agent-a' } as RouteContext['auth'],
  ...
  it('enrolls end-to-end over the route (MARVEEN_SSH_DIR seam) and audits', async () => {
    ...
    const { line, installId } = makeKeyLine()
    const r = await call(tryHandleSecurity, 'POST', '/api/security/bridge-enroll', { auth: { kind: 'token' }, body: { key_line: line, name: 'route-device-a' } })
    if (r.statusCode === 400) {
...
describe('DELETE /api/auth/device-keys/:id for a paired key', () => {
  it('drops the authorized_keys line together with the key', async () => {
    const { line, installId } = makeKeyLine()
    const outcome = await bridgeEnroll({ keyLine: line, name: 'device-c' }, testDeps())
    process.env.MARVEEN_SSH_DIR = sshDir
