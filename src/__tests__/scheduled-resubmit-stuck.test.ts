...
// The measured field case: the prompt was SUBMITTED and the session was
// actively working on it. The marker is visible in the transcript echo,
// the pane is busy, and the input box is empty. The old busy-blind check
// judged this stuck and pressed keystrokes into a working session.
const WORKING_PANE = `
  ${MARKER}
...
describe('isScheduledPromptStuck (SCHEDDUP1 invariants)', () => {
  ...
  it('marker only in scrollback is not stuck', () => {
