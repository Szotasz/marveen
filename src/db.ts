...
export function initDatabase(dbPathOverride?: string): void {
  ...
  // Migration: per-human memory isolation (multi-tenant)
  // owner_id = the human user's chat_id who caused this memory to be written.
  // NULL means the record was written before this feature shipped (treat as
  // belonging to ALLOWED_CHAT_ID for backward-compat reads).
  try {
    db.exec('ALTER TABLE memories ADD COLUMN owner_id TEXT')
  } catch {
    // column already exists
  }
  db.exec('CREATE INDEX IF NOT EXISTS idx_memories_owner ON memories(owner_id, agent_id)')
...
export interface Memory {
  ...
  owner_id: string | null  // human user identifier (chat_id); NULL = legacy/unowned
}
...
export function saveAgentMemory(
  ...
  const info = db.prepare(
    'INSERT INTO memories (chat_id, topic_key, content, sector, salience, created_at, accessed_at, agent_id, category, auto_generated, keywords, owner_id) VALUES (?, ?, ?, ?, 1.0, ?, ?, ?, ?, ?, ?, ?)'
  ).run(ALLOWED_CHAT_ID, null, content, 'semantic', now, now, agentId, category, autoGenerated ? 1 : 0, keywords ?? null, ownerId ?? null)
...
export function getAgentMemories(agentId: string, limit: number = 20, ownerId?: string): Memory[] {
  if (ownerId) {
    return db.prepare(
      "SELECT * FROM memories WHERE (agent_id = ? OR category = 'shared') AND (owner_id = ? OR owner_id IS NULL OR category = 'shared') ORDER BY accessed_at DESC LIMIT ?"
    ).all(agentId, ownerId, limit) as Memory[]
  }
  return db.prepare(
...
export function searchAgentMemories(agentId: string, query: string, limit: number = 10, ownerId?: string): Memory[] {
  const terms = buildFtsMatchExpression(query)
  if (!terms) return []
  const ownerFilter = ownerId
    ? "AND (m.owner_id = ? OR m.owner_id IS NULL OR m.category = 'shared')"
    : ''
  try {
    const params = ownerId
      ? [terms, agentId, ownerId, limit]
      : [terms, agentId, limit]
    return db.prepare(
      `SELECT m.* FROM memories m
       JOIN memories_fts f ON m.id = f.rowid
    ...
    const params = ownerId
      ? [agentId, ownerId, `%${query}%`, `%${query}%`, limit]
      : [agentId, `%${query}%`, `%${query}%`, limit]
    const ownerClause = ownerId
      ? "AND (owner_id = ? OR owner_id IS NULL OR category = 'shared')"
      : ''
    return db.prepare(
      `SELECT * FROM memories WHERE (agent_id = ? OR category = 'shared') ${ownerClause} AND (content LIKE ? OR keywords LIKE ?) ORDER BY accessed_at DESC LIMIT ?`
    ).all(...params) as Memory[]
  }
}
