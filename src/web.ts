...
import { ensureAgentHooks, ensureAgentStalenessHook, ensureEgressGate, ensureSafetyGates, ensureQuarantineReader, ensureDefaultScheduledTasks, agentSettingsPath, ensureAutonomySection } from './web/agent-scaffold.js'
  ...
  if (!hookDecision.register) {
    ...
    try {
      ...
      for (const agentName of [MAIN_AGENT_ID, ...listAgentNames()]) {
        ...
        if (ensureSafetyGates(agentName)) safetyPatched.push(agentName)
      ...
      if (safetyPatched.length) logger.info({ patched: safetyPatched }, 'safety gates (read/edit/bash/mcp/orchestration) backfilled into agent settings.json')
    } catch (err) {
