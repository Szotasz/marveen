...
- **What review looks like here:** your PR is approved the normal way, so it does carry the green "Approved" state. A PR written by the AI fleet never does, because every agent writes through the same GitHub account and GitHub refuses to let an account approve its own pull request. There the verdict is a `COMMENTED` review whose first line reads `FLEET REVIEW -- GO` or `NO-GO`. When you read the history, such a comment IS the review, not the absence of one.
