...
import tv
from gen_tv import write_xlsx

SYS = "/usr/bin/python3"
CLI = "/root/marveen/scripts/owncloud-cli.py"
SRC = "fiREG közös/temp/KEF"
DST = "fiREG közös/temp/KEF/_import_kesz/TV"
...
# file list: explicit list-file arg, else all _TV_ from the cached listing
if len(sys.argv) > 1 and os.path.exists(sys.argv[1]):
    files = [l.strip() for l in open(sys.argv[1]) if l.strip()]
else:
    files = sorted(set(re.findall(r'[A-Za-z0-9_]+_\d{4}_TV_UN\.pdf', open(CACHE).read())))
print(f"{len(files)} TV PDF {'(DRY, nincs upload)' if DRY else ''}", flush=True)

tmp = tempfile.mkdtemp(prefix="kef_tv_")
if not DRY:
...
for i, fn in enumerate(files, 1):
    ...
    try:
        ...
        write_xlsx(xp, devs)
        # quality flags (review, not failure)
        n = len(devs)
        ...
        noalt = sum(1 for d in devs if not d['altipus'])
        reasons = []
        if noflag:
        ...
        if noalt * 100 // n > 30:
            reasons.append(f"{noalt}/{n} altípus nélkül")
        if nodate * 100 // n > 20:
