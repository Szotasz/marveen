...
import tg
from gen_tg import write_xlsx

SYS = "/usr/bin/python3"
CLI = "/root/marveen/scripts/owncloud-cli.py"
SRC = "fiREG közös/temp/KEF"
DST = "fiREG közös/temp/KEF/_import_kesz/TG"
...
if len(sys.argv) > 1 and os.path.exists(sys.argv[1]):
    ...
    files = sorted(set(re.findall(r'[A-Za-z0-9_]+_\d{4}_TG_UN\.pdf', open(CACHE).read())))
print(f"{len(files)} TG PDF {'(DRY, nincs upload)' if DRY else ''}", flush=True)

tmp = tempfile.mkdtemp(prefix="kef_tg_")
if not DRY:
