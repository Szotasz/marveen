...
- Stale active row cleanup: `pruneStaleBlackboardActive(ttlHours=24)` removes `fleet_blackboard` rows stuck in `active` for longer than the TTL; called by `runDecaySweep()` as a safety net for tasks whose `sawTurn=false` path cleared the watchdog without writing done
